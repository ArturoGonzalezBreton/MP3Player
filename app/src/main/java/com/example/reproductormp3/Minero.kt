...
class Minero {
    ...
    fun mina(path: String, bdd: SQLiteDatabase): MutableList<String?> {
        ...
        File(path).walk().forEach {
            try {
                ...
                var track = lector.extractMetadata(MediaMetadataRetriever.METADATA_KEY_CD_TRACK_NUMBER)
    ...
    private fun existeArtista(artista: String?, bdd: SQLiteDatabase): Boolean {
        val cursor = bdd.rawQuery("SELECT name FROM performers WHERE ?=?",
                                        arrayOf("name", artista))
        if (cursor.count > 0) {
            cursor.close()
            return true
        }
    ...
    */
    ...
    private fun agregaAlbum(album: String?, bdd: SQLiteDatabase) {
        val albums = bdd.rawQuery("SELECT name FROM albums", emptyArray())
        val numAlbums = albums.count
        bdd.execSQL("INSERT INTO performers VALUES($numAlbums, 2, $album)")
        albums.close()
    }
    ...
    private fun agregaArtista(artista: String?, bdd: SQLiteDatabase) {
        val performers = bdd.rawQuery("SELECT name FROM performers", emptyArray())
    ...
    private fun agregaCancion(artist: String?,
        ...
        cursor = bdd.rawQuery("SELECT id_album FROM performers WHERE ?=?",
            arrayOf("name", album))
        var idAlbum: String?
        idAlbum = "null"
        if (cursor.moveToNext()) {
